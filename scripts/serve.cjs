...
function resolvePythonExe() {
  const envPython = process.env.LOCAL_AI_IMAGE_PYTHON || process.env.PYTHON;
  const candidates = [
    envPython,
  ...
  for (const candidate of candidates) {
    if ((path.isAbsolute(candidate) && fs.existsSync(candidate)) || commandWorks(candidate)) {
      return candidate;
    }
  }
...
let fallbackBackendServer = null;
...
function getGpuInfo() {
  ...
  if (osPlatform === "win32") {
    try {
      const commonPath = "C:\\Program Files\\NVIDIA Corporation\\NVSMI\\nvidia-smi.exe";
      const smi = fs.existsSync(commonPath) ? `"${commonPath}"` : "nvidia-smi";
      const output = execSync(
        `${smi} --query-gpu=name --format=csv,noheader,nounits`,
        { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] },
      ).trim().split(/\r?\n/).map(line => line.trim()).filter(Boolean);
      const nvidia = output.find(name => /nvidia|geforce|rtx|gtx/i.test(name));
      if (nvidia) {
        cachedGpuInfo = { name: nvidia };
  ...
  if (osPlatform === "win32") {
    try {
      const output = execSync(
        "powershell -NoProfile -Command \"$gpus = @(Get-CimInstance Win32_VideoController); $gpu = $gpus | Where-Object { $_.Name -match 'NVIDIA|GeForce|RTX|GTX' } | Select-Object -First 1; if (-not $gpu) { $gpu = $gpus | Select-Object -First 1 }; if ($gpu) { $gpu.Name }\"",
        { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] },
      ).trim();
      if (output) {
...
async function getHealth() {
  ...
  const ports = {
    ...
    backend: { ...(await checkPort(PORT_BACKEND)), expectedInUse: backendProc !== null || fallbackBackendServer !== null },
  };
  ...
  return {
    ...
    backend: {
      ready: backendReady,
      running: backendProc !== null || fallbackBackendServer !== null,
      error: backendError,
...
function backendAccepts(binaryPath, backendName) {
  ...
  const runtimeBackend = getRuntimeBackendName(backendName);
  try {
    const result = spawnSync(binaryPath, [
      "--backend", runtimeBackend,
      "--params-backend", runtimeBackend,
    ...
    if (output.includes("backend config failed") || output.includes(`backend '${runtimeBackend}' was not found`)) {
      return false;
    }
...
function getRuntimeBackendName(backendType) {
  if (backendType === "cuda") return "cuda0";
  if (backendType === "vulkan") return getPreferredVulkanBackendName();
  return "cpu";
}

function getPreferredVulkanBackendName() {
  if (osPlatform !== "win32") return "vulkan0";
  ...
  try {
    ...
    const nvidiaIndex = names.findIndex(name => /nvidia|geforce|rtx|gtx/i.test(name));
    if (nvidiaIndex >= 0) return `vulkan${nvidiaIndex}`;
  } catch (_) {}
...
function resolveBackendType(useGpu, backendType = "auto") {
  ...
  return available.has(requestedType) ? requestedType : options.defaultBackendType || "cpu";
}
...
function killBackend() {
  return new Promise(resolve => {
    ...
    if (fallbackBackendServer) {
      backendUnloadState = { active: true, phase: "Stopping fallback backend...", progress: 50 };
      fallbackBackendServer.close(() => {
        fallbackBackendServer = null;
        backendReady = false;
        backendUnloadState = { active: false, phase: "Backend unloaded", progress: 100 };
        if (!backendProc) resolve();
      });
...
function makeFallbackPng({ prompt = "", width = 512, height = 512, seed = -1 } = {}) {
  width = Math.max(64, Math.min(1024, parseInt(width, 10) || 512));
  height = Math.max(64, Math.min(1024, parseInt(height, 10) || 512));
  const text = String(prompt || "Local AI fallback").slice(0, 80);
  const hashInput = `${text}:${seed}`;
  let hash = 2166136261;
  for (let i = 0; i < hashInput.length; i += 1) {
    hash ^= hashInput.charCodeAt(i);
    hash = Math.imul(hash, 16777619);
  }
  const a = [hash & 255, (hash >>> 8) & 255, (hash >>> 16) & 255];
  const b = [(hash >>> 24) & 255, (hash >>> 13) & 255, (hash >>> 5) & 255];
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      const wave = (Math.sin((x + (hash & 127)) / 23) + Math.cos((y + ((hash >>> 8) & 127)) / 19)) * 18;
      const idx = row + 1 + x * 4;
      raw[idx] = Math.max(0, Math.min(255, Math.round(a[0] * (1 - t) + b[0] * t + wave)));
      raw[idx + 1] = Math.max(0, Math.min(255, Math.round(a[1] * (1 - t) + b[1] * t - wave)));
      raw[idx + 2] = Math.max(0, Math.min(255, Math.round(a[2] * (1 - t) + b[2] * t + wave / 2)));
      raw[idx + 3] = 255;
    }
  }
  ...
  ihdr[8] = 8;
  ihdr[9] = 6;
  const textChunk = Buffer.from(`Comment\0${text}`, "utf8");
  return Buffer.concat([
    Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]),
    pngChunk("IHDR", ihdr),
    pngChunk("tEXt", textChunk),
    pngChunk("IDAT", zlib.deflateSync(raw)),
    pngChunk("IEND", Buffer.alloc(0)),
  ]);
}

function startFallbackBackend(settings = {}) {
  return new Promise((resolve, reject) => {
    ...
    currentSettings.backendType = "fallback";
    currentSettings.useGpu = false;
    currentSettings.backendMode = "JS Fallback";
    currentSettings.backendBinary = "serve.cjs";
    backendLoadState = {
      active: true,
      phase: "Starting fallback backend...",
      progress: 20,
      current: 0,
      total: 0,
      speed: "",
      model: path.basename(currentSettings.model || "fallback"),
      backendMode: currentSettings.backendMode,
      backendBinary: currentSettings.backendBinary,
      device: "Node.js",
    };

    fallbackBackendServer = http.createServer(async (req, res) => {
      ...
      if (req.method === "GET" && req.url === "/v1/models") {
        return json(res, 200, { data: [{ id: path.basename(currentSettings.model || "fallback"), object: "model" }] });
      }

      if (req.method === "POST" && (req.url === "/v1/images/generations" || req.url === "/sdapi/v1/img2img")) {
        const body = await readJsonBody(req, res);
        if (!body) return;
        generationState = {
          active: true,
          step: 0,
          steps: parseInt(body.steps, 10) || currentSettings.steps || 1,
          speed: "fallback",
          decoding: false,
        };
        const size = String(body.size || `${body.width || 512}x${body.height || 512}`).match(/(\d+)x(\d+)/);
        const png = makeFallbackPng({
          prompt: body.prompt,
          width: size ? size[1] : body.width,
          height: size ? size[2] : body.height,
          seed: body.seed,
        });
        generationState = { active: false, step: 0, steps: 0, speed: "", decoding: false };
        const b64 = png.toString("base64");
        if (req.url === "/sdapi/v1/img2img") {
          return json(res, 200, { images: [b64], parameters: body, info: JSON.stringify({ seed: body.seed }) });
        }
        return json(res, 200, { created: Math.floor(Date.now() / 1000), data: [{ b64_json: b64, seed: body.seed }] });
      }

      return json(res, 404, { error: "Unknown fallback backend endpoint" });
    });

    fallbackBackendServer.once("error", err => {
      fallbackBackendServer = null;
      backendError = err.message || String(err);
      reject(err);
    });

    fallbackBackendServer.listen(PORT_BACKEND, "127.0.0.1", () => {
      backendError = null;
      markBackendReady();
      resolve();
    });
...
function startDiffusersBackend(settings = {}) {
  ...
  currentSettings = { ...currentSettings, ...settings };
  currentSettings.backendType = "diffusers";
  currentSettings.useGpu = true;
  currentSettings.backendMode = "Diffusers CUDA";
  currentSettings.backendBinary = "diffusers_backend.py";
  backendLoadState = {
    active: true,
    phase: "Loading Diffusers model...",
    progress: 10,
    current: 0,
    total: 0,
    speed: "",
  ...
  console.log("  [backend] Starting Diffusers:", args.join(" "));
  backendReady = false;
  backendProc = spawn(PYTHON_EXE, args, { stdio: "pipe", env });
  startBackendReadyPoll();

  backendProc.stdout.on("data", d => {
    const output = d.toString();
    process.stdout.write("  [diffusers] " + output);
    const cleanOutput = stripAnsi(output);
    if (cleanOutput.includes("model ready")) {
      backendLoadState.phase = "Starting API server...";
      backendLoadState.progress = 90;
    }
  ...
  backendProc.stderr.on("data", d => {
    const output = d.toString();
    process.stderr.write("  [diffusers-err] " + output);
    const cleanOutput = stripAnsi(output);
    if (cleanOutput.toLowerCase().includes("error") || cleanOutput.toLowerCase().includes("traceback")) {
  ...
  backendProc.on("exit", code => {
    backendReady = false;
    backendProc = null;
    console.log("  [diffusers] exited with code", code);
    if (code !== null && code !== 0 && !backendError) {
      backendError = `Diffusers backend exited with code ${code}`;
    }
...
async function startBackend(settings = {}) {
  ...
  if (requestedBackend === "cpu") {
    ...
    const vulkanBackend = getRuntimeBackendName("vulkan");
    args.push(
      "--backend", vulkanBackend,
      "--params-backend", vulkanBackend,
      "--rng", "cpu",
      "--sampler-rng", "cpu",
    );
  } else if (requestedBackend === "cuda") {
    const cudaBackend = getRuntimeBackendName("cuda");
    args.push(
      "--backend", cudaBackend,
      "--params-backend", cudaBackend,
      "--rng", "cuda",
      "--sampler-rng", "cuda",
    );
  }
  ...
  backendProc.stderr.on("data", d => {
    ...
    if (currentSettings.backendType === "cuda" && cleanOutput.includes("ggml_cuda_init")) {
      backendLoadState.backendMode = "CUDA GPU";
      currentSettings.backendMode = "CUDA GPU";
    }
    if (currentSettings.backendType === "vulkan" && cleanOutput.includes("ggml_vulkan")) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/api/generation-progress" && req.method === "GET") {
    if (currentSettings.backendType === "diffusers" && backendProc) {
      const proxy = http.get(`http://127.0.0.1:${PORT_BACKEND}/progress`, backendRes => {
        let data = "";
      ...
      proxy.on("error", () => json(res, 200, generationState));
      return;
    }
    return json(res, 200, generationState);
  }
